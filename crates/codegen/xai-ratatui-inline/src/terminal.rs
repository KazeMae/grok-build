    ...
    B: Backend,
{
    ...
    fn normalize_current_buffer_wide_cells(&mut self) {
        ...
        let Some(buffer) = self.buffers.get_mut(current) else {
            return;
        };
        let reset_indices = normalize_overwritten_wide_cells(buffer);
        let Some(link_ids) = self.link_ids.get_mut(current) else {
            return;
        };
...
fn normalize_overwritten_wide_cells(buffer: &mut Buffer) -> Vec<usize> {
    ...
    for row_start in (0..buffer.content.len()).step_by(row_width) {
        ...
        while index < row_end {
            let Some(cell) = buffer.content.get(index) else {
                break;
            };
            let symbol_width = cell.symbol().width();
            if symbol_width <= 1 {
                index += 1;
                continue;
            }
            ...
            let is_overwritten = buffer
                .content
                .get(index + 1..continuation_end)
                .is_some_and(|slice| slice.iter().any(|cell| cell.symbol() != " "));
