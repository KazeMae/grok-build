        ...
        if let Some(ref mut active_modal) = self.active_modal {
            ...
            } = active_modal
            {
                ...
                let picker_entries: Vec<PickerEntry> = items
                    .iter()
                    .enumerate()
                    .zip(localized_displays.iter().zip(localized_descriptions.iter()))
                    .map(|((i, _item), (label, right_label))| {
                        PickerEntry::Row(PickerRow {
                            label,
                            right_label,
                            selected: state.hovered == Some(i)
                ...
                let mut session_shortcuts: Vec<Shortcut> = if pending_delete.is_some() {
                    ...
                    if !external {
                        shortcuts.push(Shortcut {
                            label: modal_static_text(locale, "session.shortcut.expand", "e expand"),
                            clickable: false,
                            id: 0,
                        });
                    }
                    shortcuts.push(Shortcut {
                        label: modal_static_text(locale, "session.shortcut.search", "/ search"),
                        clickable: false,
                        id: 0,
                    });
                    if !chat_mode {
