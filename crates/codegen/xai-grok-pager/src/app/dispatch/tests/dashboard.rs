...
fn dashboard_begin_rename_on_workspace_row_refuses() {
    ...
    for (locale, expected) in [
        (UiLocale::EnUs, "Load the session before renaming"),
        (UiLocale::ZhCn, "重命名前请先加载会话"),
    ] {
        for row in [
            ...
            let mut app = test_app_with_agent();
            app.locale = std::sync::Arc::new(LocaleContext::new(ResolvedLocale {
                locale,
            ...
            assert_eq!(d.error_toast.as_deref(), Some(expected));
        }
    }
}
