...
fn coding_data_sharing_failed_names_the_team_admin_on_missing_acl() {
    ...
    for (ui_locale, guidance) in [
        ...
        for (error, expect_admin) in [(denial, true), ("[WKE=unauthorized:rw-required]", false)] {
            ...
            let toast = read_toast(&app);
            assert_eq!(expect_admin, toast.contains(guidance), "{error}: {toast}");
        }
    }
}
...
fn capability_tri_state_gates_banner_row_and_write() {
    ...
    let privacy_locked = xai_grok_version::coding_data_retention_locked_opt_out();
    ...
    for (capability, rollout, pick, banner, lock, writes) in [
        ...
        let banner = banner && !privacy_locked;
        let lock = if privacy_locked {
            Some(CodingDataSharingLock::PrivacyBuild)
        } else {
            lock
        };
        let writes = writes && !(privacy_locked && pick);
        let mut app = privacy_banner_ready_app();
...
#[test]
fn personal_account_respects_privacy_lock_without_a_capability() {
    let mut app = privacy_banner_ready_app();
    ...
    let privacy_locked = xai_grok_version::coding_data_retention_locked_opt_out();
    assert_eq!(!privacy_locked, app.privacy_banner_should_show());
    assert_eq!(
        privacy_locked.then_some(crate::settings::CodingDataSharingLock::PrivacyBuild),
    ...
    assert_eq!(
        ...
        effects
            .iter()
            .any(|e| matches!(e, Effect::SetCodingDataSharing { .. })),
        "personal account write follows the privacy lock: {effects:?}"
    );
}
...
fn hydrated_denial_locks_open_settings_row_and_closes_chooser() {
    ...
    assert_eq!(
        chooser_open(&app),
        (
            privacy_locked.then_some(CodingDataSharingLock::PrivacyBuild),
            !privacy_locked
        )
    );
