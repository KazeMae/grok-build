...
/// which [`top_level_label_with_locale`] falls back to `New session #<id>`. A session created on pager launch
...
fn top_level_row(id: AgentId, agent: &AgentView, pinned: bool, home: Option<&str>) -> DashboardRow {
    top_level_row_with_locale(id, agent, pinned, home, Some(agent.scrollback.locale()))
}
