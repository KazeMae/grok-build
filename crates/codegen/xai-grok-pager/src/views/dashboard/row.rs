...
use crate::views::dashboard::row_activity::{has_live_parent_activity, live_work_badges};
...
fn build_local_rows(
    ...
    for (id, agent) in agents.iter() {
        ...
        let row = top_level_row_with_locale(*id, agent, pinned.contains(&top_id), home, locale);
        rows.push(row);
