...
fn localize_next_suffix(english: &str, locale: Option<&crate::locale::LocaleContext>) -> String {
    if english == " (due now)" {
        return tasks_static(locale, "tasks.schedule.due_now", " (due now)").to_string();
    }
    if let Some(duration) = english
        .strip_prefix(" (next in ")
        .and_then(|rest| rest.strip_suffix(')'))
    {
        return tasks_text(locale, "tasks.schedule.next_in", " (next in {duration})")
...
impl TaskEntry {
    ...
    fn from_scheduled(
        ...
        let suffix = if linked_running {
            ...
            localize_next_suffix(&super::scheduled_next::next_suffix(info, now), locale)
        };
...
impl TasksPane {
    ...
    pub fn sync_with_locale(
        ...
        subagents: &HashMap<String, SubagentInfo>,
        scheduled: &HashMap<String, ScheduledTaskInfo>,
        _current_cron_task_id: Option<&str>,
        _queued_cron_ids: &std::collections::HashSet<&str>,
        workflow_runs: &[crate::views::workflows::WorkflowRunSnapshot],
        locale: Option<&crate::locale::LocaleContext>,
    ) {
        ...
        for info in scheduled.values() {
            ...
            self.items
                .push(TaskEntry::from_scheduled(info, linked, now, locale));
        }
