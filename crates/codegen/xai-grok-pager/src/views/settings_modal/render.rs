            ...
            RowEntry::Setting {
                ...
                if matches!(meta.kind, SettingKind::Group { .. }) {
                    ...
                    if is_expanded && y_cursor < area_end {
                        ...
                        let text = localized_setting_description(locale, meta);
                        render_expanded_description(buf, desc_rect, text.as_ref(), theme);
                        let consumed =
                            wrapped_description_height(text.as_ref(), area.width, desc_rect.height);
        ...
        match row {
            ...
            RowEntry::Setting {
                ...
                if matches!(meta.kind, SettingKind::Group { .. }) {
                    ...
                    if state.expanded_keys.contains(key) {
                        h = h.saturating_add(wrapped_description_height(
                            localized_setting_description(locale, meta).as_ref(),
                            area_width,
                            8,
                        ));
                    }
                ...
                if is_expanded {
                    ...
                    let text = match state.row_lock(key).map(CodingDataSharingLock::reason) {
                        Some(reason) => localized_lock_reason(locale, reason),
                        None => localized_setting_description(locale, meta),
                    };
                    h = h.saturating_add(wrapped_description_height(text.as_ref(), area_width, 8));
                }
                heights.push(h);
            }
        }
