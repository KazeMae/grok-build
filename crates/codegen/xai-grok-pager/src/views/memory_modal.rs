...
fn empty_state_markdown(
    capture_enabled: bool,
    dream_enabled: bool,
    locale: Option<&LocaleContext>,
) -> String {
    let mut text = memory_text(
        locale,
        "memory.empty.heading",
        "**Nothing remembered yet.**\n\n",
    );
    if capture_enabled {
        text.push_str(&memory_text(
            locale,
            "memory.empty.auto_save",
    ...
    if dream_enabled {
        text.push_str(&memory_text(
            locale,
            "memory.empty.dream",
            "- `/dream` organizes saved notes into topics.\n",
        ));
    }
    text.push_str(&memory_text(
        locale,
...
fn disabled_state_markdown(
    reason: Option<MemoryDisabledReason>,
    locale: Option<&LocaleContext>,
) -> String {
    match reason {
        None | Some(MemoryDisabledReason::SessionToggle) => memory_text(
            locale,
            "memory.disabled.session_toggle",
            "\
...
to repeat yourself.",
        ...
        Some(MemoryDisabledReason::ConfigOptOut) => memory_text(
            locale,
            "memory.disabled.config_opt_out",
            "\
...
there (or remove the line) to keep memory on. Anything already remembered is kept on disk.",
        ...
        Some(MemoryDisabledReason::ProcessDisabled) => memory_text(
            locale,
            "memory.disabled.process_disabled",
            "\
...
remembered is kept on disk.",
        ...
        Some(MemoryDisabledReason::RolloutRestricted) => memory_text(
            locale,
            "memory.disabled.rollout_restricted",
            "\
...
can't be turned on here. Press **s** for details.",
        ...
        Some(MemoryDisabledReason::NotConfigured) => memory_text(
            locale,
            "memory.disabled.not_configured",
            "\
...
No memory storage is set up for this session, so there is nothing to browse or turn on.",
        ...
        Some(MemoryDisabledReason::Unknown) => memory_text(
            locale,
            "memory.disabled.unknown",
            "\
...
impl MemoryModalState {
    ...
    fn load_preview(&mut self) {
        ...
        self.preview_markdown = path.and_then(|path| match read_note(&path, MAX_PREVIEW_BYTES) {
            ...
            NoteRead::TooLarge => Some(MarkdownContent::new(memory_text(
                None,
                "memory.preview.too_large_markdown",
...
pub fn render_memory_modal_with_locale(
    ...
    if state.shows_notice() {
        ...
        let markdown = if state.memory_enabled {
            empty_state_markdown(state.capture_enabled, state.dream_enabled, locale)
        } else {
            disabled_state_markdown(state.disabled_reason, locale)
        };
...
fn render_preview_with_locale(
    ...
    let Some(markdown) = state.preview_markdown.as_ref() else {
        ...
        if !state.filter_has_no_matches() {
            let msg = memory_static(locale, "memory.preview.no_file", "No file selected");
            let style = Style::default().fg(theme.gray_dim).bg(theme.bg_base);
...
fn build_shortcuts_with_locale(
    ...
    match &state.mode {
        MemoryModalMode::Browse if !state.memory_enabled => {
            ...
            if state.can_enable() {
                shortcuts.push(plain(memory_static(
                    locale,
                    "memory.shortcut.turn_on",
                    "t turn on",
                )));
            }
            ...
            shortcuts.push(plain(memory_static(
                locale,
                "memory.shortcut.close",
                "Esc close",
            )));
            shortcuts
        }
        MemoryModalMode::Browse if !state.has_notes() => vec![
            plain(memory_static(
                locale,
                "memory.shortcut.turn_off",
                "t turn off",
            )),
            plain(fullscreen_label),
            plain(memory_static(locale, "memory.shortcut.close", "Esc close")),
        ],
        MemoryModalMode::PreviewFocused => vec![
            plain(memory_static(
                locale,
                "memory.shortcut.scroll",
                "\u{2191}/\u{2193} scroll",
            )),
            plain(memory_static(
                locale,
                "memory.shortcut.drag_to_copy",
                "drag to copy",
            )),
            plain(memory_static(
                locale,
                if state.split_shown {
                    "memory.shortcut.esc_list"
                } else {
                    "memory.shortcut.esc_back"
                },
                if state.split_shown {
                    "Esc list"
                } else {
        ...
        MemoryModalMode::Browse => {
            let mut shortcuts = vec![
                ...
                plain(memory_static(locale, "memory.shortcut.search", "/ search")),
                plain(memory_static(
                    locale,
                    if state.split_shown {
                        "memory.shortcut.enter_read"
                    } else {
                        "memory.shortcut.enter_open"
                    },
                    if state.split_shown {
                        "Enter read"
                    } else {
