...
impl<'e> BucketAccumulator<'e> {
    ...
    fn into_label(
        ...
        for (i, bucket) in self.buckets.iter().enumerate() {
            ...
            let localize_verb = |running: bool| {
                let english = bucket.kind.verb(running);
                locale
                    .map(|locale| locale.named_text(bucket.kind.verb_locale_key(running), english))
                    .unwrap_or_else(|| std::borrow::Cow::Borrowed(english))
            };
            let localize_noun = |n: usize| {
                let english = bucket.kind.noun(n);
                locale
                    .map(|locale| locale.named_text(bucket.kind.noun_locale_key(), english))
                    .unwrap_or_else(|| std::borrow::Cow::Borrowed(english))
            };
            ...
            let segment = match bucket.kind {
                VerbGroupKind::Subagent => {
                    ...
                    if running_n > 0 && done_n > 0 {
                        let verb = localize_verb(true);
                        let noun = localize_noun(running_n);
                        let inner_sep = locale
                            .map(|locale| {
                                locale.named_text("scrollback.verb_group.separator", ", ")
                            })
                            .unwrap_or_else(|| std::borrow::Cow::Borrowed(", "));
                        let completed = locale
                            .map(|locale| {
                                locale.named_text("scrollback.verb_group.completed", "completed")
                            })
                            .unwrap_or_else(|| std::borrow::Cow::Borrowed("completed"));
                        format!(
                            "{separator}{verb} {running_n} {noun}{inner_sep}{done_n} {completed}"
                        )
                    } else {
                        let verb = localize_verb(running_n > 0);
                        let noun = localize_noun(count);
                        format!("{separator}{verb} {count} {noun}")
                    }
                }
                _ => {
                    let verb = localize_verb(self.running);
                    let noun = localize_noun(count);
                    format!("{separator}{verb} {count} {noun}")
                }
            };
