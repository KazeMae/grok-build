...
fn zh_localization_sent_message_translates_fixed_chrome_only() {
    let block = sent(unresolved("sub-123", "literal payload"));
    let mut ctx = context(120, DisplayMode::Expanded);
    ctx.locale = zh_locale();
    let rendered = block
        .output(&ctx)
        .lines
        .iter()
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("子智能体 ID：sub-123"), "{rendered}");
    assert!(rendered.contains("literal payload"), "{rendered}");
}
...
fn zh_localization_translates_only_product_owned_delivery_reasons() {
    let mut ctx = context(120, DisplayMode::Expanded);
    ctx.locale = zh_locale();
    let fallback = SentMessageToolCallBlock::new(
        SentMessagePresentation::Rejected {
            reason: "Message was not accepted or delivery details are unavailable.".into(),
        },
        None,
    );
    let fallback_text = fallback
        ...
        .join("\n");
    assert!(
        fallback_text.contains("消息未被接受，且无法获取送达详情。"),
        "{fallback_text}"
    );

    let saturated = SentMessageToolCallBlock::new(
        SentMessagePresentation::Rejected {
            reason: "Message admission is saturated (maximum 7 in flight).".into(),
        },
        None,
    );
    let saturated_text = saturated
        ...
        .join("\n");
    assert!(
        saturated_text.contains("消息接收已饱和（最多允许 7 条消息同时处理中）。"),
        "{saturated_text}"
    );

    let limit = SentMessageToolCallBlock::new(
        SentMessagePresentation::Rejected {
            reason: "Message size is invalid: observed 2048 bytes; maximum is 1024 bytes.".into(),
        },
        None,
    );
    let limit_text = limit
        ...
        .join("\n");
    assert!(
        limit_text.contains("消息大小无效：实际 2048 字节；上限为 1024 字节。"),
        "{limit_text}"
    );
    ...
    let dynamic_text = dynamic
        ...
        .join("\n");
    assert!(
        dynamic_text.contains("opaque provider reason"),
        "{dynamic_text}"
    );
}
