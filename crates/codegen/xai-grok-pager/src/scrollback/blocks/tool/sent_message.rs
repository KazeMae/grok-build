...
pub const UNAVAILABLE_DELIVERY_REASON: &str =
...
impl SentMessageTarget {
    ...
    fn noun_with_locale<'a>(&'a self, locale: &'a crate::locale::LocaleContext) -> Cow<'a, str> {
        match self {
            Self::Named { label, .. } => scrub_display(label),
            Self::Parent => Cow::Borrowed(
                locale.named_static_text("scrollback.sent_message.noun.parent", PARENT_NOUN),
            ),
            Self::Unresolved { subagent_id } => {
                let id = scrub_display(subagent_id);
                let fallback = locale
                    .named_static_text("scrollback.sent_message.noun.subagent", FALLBACK_NOUN);
                match id.char_indices().nth_back(SHORT_ID_CHARS) {
                    Some((index, ch)) => {
                        let (_, tail) = id.split_at(index + ch.len_utf8());
                        Cow::Owned(format!("{fallback} \u{2026}{tail}"))
                    }
                    None if id.is_empty() => Cow::Borrowed(fallback),
                    None => Cow::Owned(format!("{fallback} {id}")),
                }
            }
        }
    }
    ...
    fn detail_with_locale<'a>(
        ...
        match self {
            ...
            Self::Rejected { reason } => {
                Some((localized_reason(locale, reason), MessageDetailStyle::Error))
            }
            Self::Unconfirmed { reason } => Some((
                localized_reason(locale, reason),
                MessageDetailStyle::Warning,
            )),
        }
    }
...
impl SentMessageToolCallBlock {
    ...
    fn verb_and_target_with_locale(&self, locale: &crate::locale::LocaleContext) -> String {
        let noun = self.input.as_ref().map_or_else(
            || {
                Cow::Borrowed(
                    locale
                        .named_static_text("scrollback.sent_message.noun.subagent", FALLBACK_NOUN),
            ...
            |input| input.target.noun_with_locale(locale),
        );
        let phrase = |id: &str, english: &str| {
            locale
                .named_text(id, english)
                .replace("{noun}", noun.as_ref())
        };
        match &self.presentation {
            SentMessagePresentation::Sending => {
                phrase("scrollback.sent_message.verb.sending", "sending to {noun}")
            }
            SentMessagePresentation::Sent => {
                match self.input.as_ref().and_then(|input| input.delivery) {
                    None | Some(SentMessageDelivery::Steer) => {
                        phrase("scrollback.sent_message.verb.sent", "sent to {noun}")
                    }
                    Some(SentMessageDelivery::Queue) => {
                        phrase("scrollback.sent_message.verb.queued", "queued for {noun}")
                    }
                    Some(SentMessageDelivery::Interject) => phrase(
                        "scrollback.sent_message.verb.interjected",
                        "interjected to {noun}",
                    ),
                }
            }
            SentMessagePresentation::Rejected { .. } => phrase(
                "scrollback.sent_message.verb.rejected",
                "rejected \u{00b7} {noun}",
            ),
            SentMessagePresentation::Unconfirmed { .. } => phrase(
                "scrollback.sent_message.verb.unconfirmed",
                "unconfirmed \u{00b7} {noun}",
            ),
        }
    }
    ...
    fn collapsed_line(&self, ctx: &BlockContext, theme: &Theme) -> Line<'static> {
        ...
        let label = ctx
            .locale
            .named_static_text("scrollback.sent_message.header_label", HEADER_LABEL);
        Line::from(vec![
            Span::styled(label, label_style),
            Span::styled(self.verb_and_target_with_locale(&ctx.locale), theme.muted()),
        ])
    }

    /// Never muted. Suffixes name the requested delivery and, for a finished send that stalled, its admission time.
    fn expanded_header(&self, ctx: &BlockContext, theme: &Theme) -> Line<'static> {
        let label = ctx
            .locale
            .named_static_text("scrollback.sent_message.header_label", HEADER_LABEL);
        let mut spans = vec![
            Span::styled(label, theme.primary().add_modifier(Modifier::BOLD)),
            Span::styled(
                self.verb_and_target_with_locale(&ctx.locale),
                theme.primary(),
            ),
        ];
...
/// Product-owned delivery reasons stay localizable; opaque provider text is copied as-is.
fn localized_reason<'a>(locale: &'a crate::locale::LocaleContext, reason: &'a str) -> Cow<'a, str> {
    let text = |id: &str, english: &str| locale.named_text(id, english).into_owned();
    if reason == UNAVAILABLE_DELIVERY_REASON {
        return Cow::Owned(text(
            "scrollback.sent_message.unavailable_reason",
            UNAVAILABLE_DELIVERY_REASON,
        ));
    }
    if reason == "Subagent not found or not owned by this session." {
        return Cow::Owned(text(
            "scrollback.sent_message.reason.not_found_or_not_owned",
            reason,
        ));
    }
    if reason == "Subagent is not active or is finalizing." {
        return Cow::Owned(text(
    ...
    if reason == "Message was not accepted before the delivery deadline." {
        return Cow::Owned(text(
            "scrollback.sent_message.reason.not_accepted_before_deadline",
            reason,
        ));
    }
    if reason == "Active agent messages are unsupported in this context." {
        return Cow::Owned(text("scrollback.sent_message.reason.unsupported", reason));
    }
    if reason == "Message was not accepted because the subagent channel closed." {
        return Cow::Owned(text(
            "scrollback.sent_message.reason.channel_closed",
            reason,
        ));
    }
    if let Some(max_in_flight) = reason
        .strip_prefix("Message admission is saturated (maximum ")
        .and_then(|rest| rest.strip_suffix(" in flight)."))
    {
        return Cow::Owned(
            text(
            ...
            .replace("{max_in_flight}", max_in_flight),
        );
    }
    if let Some(rest) = reason.strip_prefix("Message size is invalid: observed ")
        && let Some((observed_bytes, rest)) = rest.split_once(" bytes; maximum is ")
        && let Some(max_bytes) = rest.strip_suffix(" bytes.")
    {
        return Cow::Owned(
            text(
            ...
            .replace("{observed_bytes}", observed_bytes)
            .replace("{max_bytes}", max_bytes),
        );
    }
    Cow::Borrowed(reason)
}
...
impl BlockContent for SentMessageToolCallBlock {
    fn output(&self, ctx: &BlockContext) -> BlockOutput {
        ...
        let mut lines: Vec<BlockLine> = vec![self.expanded_header(ctx, &theme).into()];
        ...
        if let Some(raw_id) = self.input.as_ref().and_then(|input| input.target.raw_id()) {
            ...
            let id_wrap = RtOptions::new(width)
                .initial_indent(Line::from(Span::styled(id_label, theme.muted())));
