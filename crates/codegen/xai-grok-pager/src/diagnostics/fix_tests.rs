...
fn canonical_and_short_ids_resolve_to_canonical_id() {
    ...
    assert_eq!(command, "grok-zh doctor fix ssh-wrap");
    assert_eq!(
        resolve_fix_id(command.strip_prefix("grok-zh doctor fix ").unwrap()).unwrap(),
        SSH_WRAP_ID
    );
...
fn tmux_fix_registry_resolves_every_short_and_canonical_id() {
    for (id, handle, _) in automatic_fix_choices() {
        ...
        assert_eq!(
            ...
            format!("grok-zh doctor fix {handle}")
        );
    }
}
...
fn tmux_managed_items_coexist_and_each_apply_is_one_transaction() {
    ...
    assert_eq!(content.matches("# >>> grok-zh doctor >>>").count(), 1);
    for id in [
        TMUX_CLIPBOARD_ID,
        DCS_PASSTHROUGH_ID,
        TMUX_EXTENDED_KEYS_ID,
...
fn bash_zsh_and_fish_plans_use_exact_paths_and_aliases() {
    ...
    for (shell, relative, alias) in [
        ("/bin/bash", ".bashrc", "alias ssh='grok-zh wrap ssh'"),
        ("/bin/zsh", ".zshrc", "alias ssh='grok-zh wrap ssh'"),
        (
            "/usr/local/bin/fish",
...
fn shell_aliases_expand_to_exact_argv_and_bypass_is_explicit() {
    ...
    let grok = temp.path().join("grok-zh");
    std::fs::write(
        &grok,
        format!(
            "#!/bin/sh\nprintf '%s\\n' \"$@\" > '{}'\n",
    ...
    if let Some(bash) = find_on_path("bash") {
        ...
        std::fs::write(&rc, "alias ssh='grok-zh wrap ssh'\n").unwrap();
        let command = format!(
    ...
    let mut shell = std::process::Command::new(bash);
    shell
        .args(["-ic", "alias ssh='grok-zh wrap ssh'; command ssh host"])
        .env("CAPTURE", &capture)
        .env(
            "PATH",
            format!(
