...
pub struct SessionModelSwitch {
    ...
    /// Lossy compact at switch end. This fork sets the flag when `/model` lands on
    /// a Claude-named model from anything else (catalog id / slug / display name
    /// contains `"claude"`). Upstream `model_family` is still omitted on custom
    /// uniapi entries, so family inequality is not enough.
