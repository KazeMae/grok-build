...
async fn connect_leader_control(
    ...
    .map_err(|e| {
        anyhow::anyhow!(localized_cli_template(
            ...
            "no running leader for this environment ({error}). Start a grok session, or run `{command} start`.",
            &[("{error}", &e.to_string()), ("{command}", door.name)],
        ))
    })
}
...
async fn spawn_and_connect_leader(
    ...
    drop(conn);
    connect_leader_control(&agent_config, &LeaderTargetArgs::default(), door, locale).await
}
#[tracing::instrument(level = "debug", skip_all)]
async fn workspace_control(
    ...
    let agent_config = xai_grok_shell::config::load_agent_config_disk_only().map_err(|e| {
        anyhow::anyhow!(localized_cli_template(
            locale,
            "cli.workspace.config.agent_failed",
            "Failed to create agent config: {error}",
    ...
    let client = connect_leader_control(&agent_config, target, &WORKSPACE_DOOR, locale).await?;
...
async fn async_main(
        ...
        match command {
            ...
            Command::Login {
                ...
                let config = xai_grok_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                let authenticated = match xai_grok_login::run_cli_login(
                ...
                .await
                {
                    ...
                    Err(error) => {
                        ...
                        if error.to_string() == DEVBOX_AUTHORITY_ERROR {
                            anyhow::bail!(
                                "{}",
                                startup_locale.named_static_text(
                                    "cli.login.devbox_authority_unsupported",
                                    DEVBOX_AUTHORITY_ERROR,
                                )
                            );
                        }
                        return Err(error);
    ...
    match result {
        Ok(true) => {
            ...
            let cli = xai_grok_product::CLI_NAME;
            if finish_update_on_exit(adopted, &update_config).await {
                eprintln!("更新已安装。运行 `{cli}` 启动。");
            } else {
                eprintln!("更新未完成。运行 `{cli} update` 重试。");
            }
            Ok(())
        }
        Ok(false) => Ok(()),
