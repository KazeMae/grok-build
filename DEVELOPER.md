# 开发者指南（grok-zh overlay）

本仓库是 KazeMae 维护的 grok-build fork，叠了 GrokZen 的简体中文界面与编译期隐私开关。
**没有**移植 GrokZen 的社区自动更新通道和各平台安装器；`grok update` 仍走官方 x.ai 通道。

普通用户请阅读根目录 [`README.md`](README.md) 与 [`FORK.md`](FORK.md)。

## 社区 overlay 约定

- 程序名：`grok-zh`（与官方 `grok` 可并存）；数据目录仍是 `~/.grok` / `GROK_HOME`。
...
./target/release/grok-zh --version
